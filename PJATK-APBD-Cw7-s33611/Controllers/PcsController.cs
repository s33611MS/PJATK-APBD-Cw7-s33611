...
public class PcsController(IPcService service) : ControllerBase
{
    ...
    public async Task<IActionResult> GetComponentsById([FromRoute] int id, CancellationToken cancellationToken)
    {
        try
        {
            return Ok(await service.GetComponentsByIdAsync(id, cancellationToken));
        }
        catch (NotFoundException e)
        {
            return NotFound(e.Message);
    ...
    [HttpPost]
    public async Task<IActionResult> Add([FromBody] CreatePcDto request, CancellationToken cancellationToken)
    {
    ...
    [HttpPut("{id:int}")]
    public async Task<IActionResult> Update([FromRoute] int id, [FromBody] UpdatePcDto request, CancellationToken cancellationToken)
    {
        try
        {
